...
from views.authentication.security_question_view import SecurityQuestionRegisterView
from viewmodels.authentication.security_question_viewmodel import SecurityQuestionRegisterViewModel
from widgets.info_panel import *
from models.authentication.authentication import Method
import json


class TestSecurityQuestionRegisterView(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def setUp(self, note_file_path, simulation_note_path, user_file_path):
        ...
        self.view = SecurityQuestionRegisterView(viewmodel=SecurityQuestionRegisterViewModel(), info_panel=self.info_panel, parent=self.parent, ui="src/views_ui/register_views/security_question_view.ui")
        self.parent.show()
    ...
    def test_info_panel(self):
        ...
        self.assertEqual(self.info_panel.measure_bar.value(), 40)
        
    def test_update_state(self):
        self.view.update_state("test", 0)
        self.assertFalse(self.view.submit_btn.isEnabled())
        self.assertFalse(self.view.remove_btn.isEnabled())
        self.assertFalse(self.view.add_question_btn.isEnabled())
        self.assertEqual(self.view.warning_label.text(), "test")

    def test_update_data(self):
        self.view.update_data({"timestamp_register": "", "user_questions": "", 
         "user_answers": "", "hashed_secret": "", "salt": ""}, 0)
