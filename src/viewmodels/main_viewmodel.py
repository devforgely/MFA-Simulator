...
class MainViewModel(QObject):
    ...
    info_bar_added = pyqtSignal(InfoBarIcon, str)
    ...
    def on_message(self, message_title: str, *args: Any) -> None:
        if message_title == "Update Coins":
            ...
            if args[1]: # bool to determine whether if you have earned coins or not
                self.add_info_bar(InfoBarIcon.COIN, "You've just earned some coins!")
        elif message_title == "Insufficient Coins":
            self.add_info_bar(InfoBarIcon.WARNING, f"Requires {args[0]} more coins.")
        elif message_title == "Update Badges":
            self.badge_count_changed.emit(f"{args[0][0]}/{args[0][1]}")
            self.add_info_bar(InfoBarIcon.MEDAL, f"You've been awarded with {args[1]} MFA badge!")
        elif message_title == "Success Notification":
            self.add_info_bar(InfoBarIcon.SUCCESS, args[0])
        elif message_title == "Warning Notification":
            self.add_info_bar(InfoBarIcon.WARNING, args[0])
        elif message_title == "Error Notification":
            self.add_info_bar(InfoBarIcon.ERROR, args[0])
    ...
    def add_info_bar(self, icon: InfoBarIcon, content: str) -> None:
        if self.is_show_notification:
            self.info_bar_added.emit(icon, content)
